...
public class PersonalSectionsControllerTests extends ControllerTestCase {
  ...
  public void api_psid_sections__user_logged_in__returns_existing_course() throws Exception {
	...
	@Test
	public void api_deleteScheduleAndLectures__user_logged_in__successful_deletion() throws Exception {
		User u = currentUserService.getCurrentUser().getUser();
		PersonalSchedule ps = PersonalSchedule.builder()
				.name("Name 1")
				.description("Description 1")
				.quarter("20221")
				.user(u)
		...
		when(ucsbCurriculumService.getAllSections(eq("59501"), eq("20221")))
				.thenReturn("{\"classSections\": [{\"enrollCode\": \"59501\"}, {\"enrollCode\": \"59502\"}]}");
		MvcResult response = mockMvc.perform(delete("/api/personalSections/delete")
				.param("psId", "13")
		...
		String responseString = response.getResponse().getContentAsString();
		assertEquals("{\"message\":\"Schedule with psId 13 and associated lectures with enrollCd 59501 deleted\"}",
				responseString);
	}

	@WithMockUser(username = "user", roles = { "USER" })
	@Test
	public void api_deleteScheduleAndLectures__user_logged_in__unauthorized_error_from_service() throws Exception {
		User u = currentUserService.getCurrentUser().getUser();
		PersonalSchedule ps = PersonalSchedule.builder()
				.name("Name 1")
				.description("Description 1")
