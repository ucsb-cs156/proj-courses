...
describe("AdminJobsPage tests", () => {
  ...
  test("user can submit the update grade history job", async () => {
    render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <AdminJobsPage />
    ...
    expect(await screen.findByText("Update Grade History")).toBeInTheDocument();

    const updateCoursesButton = screen.getByText("Update Grade History");
    expect(updateCoursesButton).toBeInTheDocument();
    updateCoursesButton.click();

    expect(await screen.findByTestId("TestJobForm-fail")).toBeInTheDocument();

    const submitButton = screen.getByTestId("updateGradeHistory");
