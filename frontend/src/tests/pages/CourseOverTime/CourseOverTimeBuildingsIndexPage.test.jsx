...
import { coursesInLib, coursesInLibDifferentRoom } from "fixtures/buildingFixtures";
...
test("filters sections by classroom when classroom is selected", async () => {
  ...
  await waitFor(() => {
    expect(screen.getByText((text) => text.includes("184"))).toBeInTheDocument();
  });
});
...
test("shows all sections when classroom is ALL", async () => {
  ...
  await waitFor(() => {
    expect(screen.getByText((text) => text.includes("184"))).toBeInTheDocument();
    expect(screen.getByText((text) => text.includes("284"))).toBeInTheDocument();
  });
});
...
test("filters out sections not in selected classroom", async () => {
  ...
  await waitFor(() => {
    expect(screen.queryByText((text) => text.includes("184"))).not.toBeInTheDocument();
    expect(screen.getByText((text) => text.includes("284"))).toBeInTheDocument();
  });
});
...
test("filters sections with multiple timeLocations correctly", async () => {
  ...
  await waitFor(() => {
    expect(screen.getByText((text) => text.includes("184"))).toBeInTheDocument();
    expect(screen.queryByText((text) => text.includes("284"))).not.toBeInTheDocument();
  });
});
