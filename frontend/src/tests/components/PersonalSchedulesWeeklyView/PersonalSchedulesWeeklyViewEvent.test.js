...
describe('PersonalSectionsEvents tests', () => {
    ...
    test('renders event with correct styles and popover', async () => {
        ...
        await waitFor(() => expect(screen.getByTestId('PersonalSectionsEvent-description')).toBeInTheDocument());

    });


    const heights = [
        { startTime: '12:00', endTime: '12:10', expectedFontSize: null, expectedHeight: 10 },
        { startTime: '12:00', endTime: '12:20', expectedFontSize: '10px', expectedHeight: 20 },
        { startTime: '12:00', endTime: '12:30', expectedFontSize: '12px', expectedHeight: 30 },
        { startTime: '12:00', endTime: '12:40', expectedFontSize: '14px', expectedHeight: 40 },
    ...
    heights.forEach(({ startTime, endTime, expectedFontSize, expectedHeight }) => {
        test(`renders event with height from ${startTime} to ${endTime} with font size ${expectedFontSize}`, async () => {
            ...
            if(expectedFontSize === null) {
                expect(screen.queryByTestId('PersonalSectionsEvent-title')).not.toBeInTheDocument();
            }
            else {
            ...
            if(expectedHeight >= 40) {
                expect(screen.getByTestId('PersonalSectionsEvent-title')).toBeInTheDocument();
                ...
                expect(screen.queryByTestId('PersonalSectionsEvent-title')).toBeInTheDocument();
                expect(screen.queryByTestId('PersonalSectionsEvent-time')).not.toBeInTheDocument();
            }
            else {
                expect(screen.queryByTestId('PersonalSectionsEvent-title')).not.toBeInTheDocument();
                expect(screen.queryByTestId('PersonalSectionsEvent-time')).not.toBeInTheDocument();
