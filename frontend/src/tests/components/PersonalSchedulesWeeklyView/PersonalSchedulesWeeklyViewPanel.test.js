...
describe('PersonalSchedulesWeeklyViewPanel tests', () => {
    ...
    test('renders without crashing', async () => {
        ...
        daysOfWeek.forEach(day => {
            const dayTitle = screen.getByTestId(`PersonalSchedulesWeeklyView-${day}-title`);
        ...
        hours.forEach(hour => {
            const hourSlot = screen.getByTestId(`PersonalSchedulesWeeklyView-${hour.replace(' ', '-')}-title`);
            expect(hourSlot).toBeInTheDocument();
            const { getByTestId } = within(hourSlot);
            expect(getByTestId(`PersonalSchedulesWeeklyView-${hour.replace(' ', '-')}-label`)).toBeInTheDocument();
        });

        expect(screen.getAllByTestId('PersonalSchedulesWeeklyView-base-slot').length).toBe(161);
    });
    ...
    test('renders events on correct days', async () => {
        ...
        events.forEach(event => {
            expect(screen.getAllByTestId(`PersonalSectionsEvent-${event.id}`).length).toBe(1);
            const dayColumn = screen.getByTestId(`PersonalSchedulesWeeklyView-${event.day}-column`);
            expect(dayColumn).toBeInTheDocument();
            const { getByTestId } = within(dayColumn);
            expect(getByTestId(`PersonalSectionsEvent-${event.id}`)).toBeInTheDocument();
        });
    });
